//frontend/src/hooks/use-keywords-api.ts
import { useState, useEffect, useCallback } from 'react';
import { toast } from 'react-toastify';
import * as keywordsApi from '@/lib/api/keywords-api';
import {
  UploadResponse,
  FilterRanges,
  FilterResponse,
...
export function useKeywordsApi() {
  ...
  useEffect(() => {
    const checkApiHealth = async () => {
      try {
        await keywordsApi.checkKeywordApiHealth();
      } catch (error) {
        toast.error('无法连接到关键词API服务器。请确认后端服务是否正常运行。');
      }
    };
  ...
  const fetchBrandOverlap = useCallback(async () => {
    ...
    try {
      const data = await keywordsApi.getKeywordBrandOverlap();
      setBrandOverlapData(data);
      return data;
    } catch (error) {
      console.error('获取关键词品牌重叠数据错误:', error);
      // 后台操作，不显示 toast 提示
      return null;
    } finally {
  ...
  const fetchFilterRanges = useCallback(async () => {
    ...
    try {
      const data = await keywordsApi.getKeywordFilterRanges();
      setFilterRanges({
        ...data,
      ...
      console.error('获取关键词过滤范围错误:', error);
      // 后台操作，不显示 toast 提示
      return null;
    } finally {
  ...
  const getExportUrl = useCallback(() => {
    return keywordsApi.exportKeywordData();
  }, []);

  // 获取唯一导出 URL
  const getExportUniqueUrl = useCallback(() => {
    return keywordsApi.exportUniqueKeywordData();
  }, []);
